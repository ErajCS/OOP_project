...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(800, 470), "Main Menu");

    // Load the background image
    sf::Texture backgroundTexture;
    if (!backgroundTexture.loadFromFile("bunny_run.png")) {
    ...
    startButton.setPosition(312, 330);                    // Button position

    sf::Text startText("PLAY", font, 30);
    startText.setFillColor(sf::Color::Black);
    startText.setPosition(startButton.getPosition().x +35, startButton.getPosition().y + 5);

    // Create Exit button
    sf::RectangleShape exitButton(sf::Vector2f(150, 50));
    exitButton.setFillColor(sf::Color(180, 50, 80));
    exitButton.setPosition(315, 400);

    sf::Text exitText("EXIT", font, 30);
    exitText.setFillColor(sf::Color::Black);
    exitText.setPosition(exitButton.getPosition().x + 35, exitButton.getPosition().y + 10);
